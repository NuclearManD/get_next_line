...
char			*get_file_buffer(t_list **buffers, const int fd)
{
	t_list				*lst;
...
void			file_buffer_free(t_list **buffers, const int fd)
{
	t_list				*to_del;
...
int				get_line_rd_lopen(const int fd, char **line, char *buffer)
{
	char				*next;
	char				*tmp;
	int					i;
